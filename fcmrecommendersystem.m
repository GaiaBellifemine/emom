...
emotionIndex = getUserEmotion(); % Ottiene l'emozione selezionata dall'utente
dataset = predictRatings(dataset); % Predice i rating delle canzoni basandosi su popolarità e valutazioni precedenti
dataset = generatePlaylist(dataset, emotionIndex); % Genera una playlist in base all'emozione e ad altre caratteristiche
dataset = displayAndRateSongs(dataset, datasetStringFormat); % Mostra le canzoni candidate e permette all'utente di valutarle
updateDataset(dataset); % Aggiorna il dataset con le nuove valutazioni

%% Fase di input dell'emozione dell'utente
function emotion = getUserEmotion()
...
global numRigheDataset;
	...
	switch(emotionIndex)
		case 1 % happiness
			for i=1:numRigheDataset
				if (((dataset(i,9)==2) || (dataset(i,9)==4)) && dataset(i,6) >= sogliaDanceability) % controllo se la colonna 9 del dataset ha il cluster numero 2 o 4 al suo interno
		...
		case 2 % sadness
			%cluster = 2;
			for i=1:numRigheDataset
				if ((dataset(i,9)==5 || dataset(i,9)==3) && dataset(i,6) < sogliaDanceability) % controllo se la Danceability e' < o > di sogliaDanceability)
					dataset(i,12) = addToPlayList; % aggiorno il dataset con la nuova informazione: 1 = la canzone va fara' parte del sottoinsieme di possibili brani da raccomandare all'utente
				end
			end
		case 3 % hanger
			for i=1:numRigheDataset
				if ((dataset(i,9)==2 && dataset(i,6) < sogliaDanceability) || ((dataset(i,9)==5 || dataset(i,9)==3)&& dataset(i,6) >= sogliaDanceability))
					dataset(i,12) = addToPlayList; % aggiorno il dataset con la nuova informazione: 1 = la canzone va fara' parte del sottoinsieme di possibili brani da raccomandare all'utente
				end
			end
		case 4 % excitement
			for i=1:numRigheDataset
				if (dataset(i,9)==1 && dataset(i,6) >= sogliaDanceability)
					dataset(i,12) = addToPlayList; % aggiorno il dataset con la nuova informazione: 1 = la canzone va fara' parte del sottoinsieme di possibili brani da raccomandare all'utente
				end
			end
		case 5 % calmness
			for i=1:numRigheDataset
				if ((dataset(i,9)==1 || dataset(i,9)==4) && dataset(i,6) < sogliaDanceability)
					dataset(i,12) = addToPlayList; % aggiorno il dataset con la nuova informazione: 1 = la canzone va fara' parte del sottoinsieme di possibili brani da raccomandare all'utente
				end
			end
		otherwise
